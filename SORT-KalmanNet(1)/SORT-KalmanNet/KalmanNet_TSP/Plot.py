...
class Plot:
    ...
    def NNPlot_epochs(self, N_Epochs_plt, MSE_KF_dB_avg,
        ...
        plt.plot(x_plt, y_plt1.cpu().numpy(), KColor[0], label=Klegend[0])

        # CV
        y_plt2 = MSE_cv_dB_epoch[range(0, N_Epochs_plt)]
        plt.plot(x_plt, y_plt2.cpu().numpy(), KColor[1], label=Klegend[1])

        # Test
        y_plt3 = MSE_test_dB_avg * torch.ones(N_Epochs_plt)
        plt.plot(x_plt, y_plt3.cpu().numpy(), KColor[2], label=Klegend[2])

        # KF
        y_plt4 = MSE_KF_dB_avg * torch.ones(N_Epochs_plt)
        plt.plot(x_plt, y_plt4.cpu().numpy(), KColor[3], label=Klegend[3])
    ...
    def NNPlot_Hist(self, MSE_KF_data_linear_arr, MSE_KN_linear_arr):
        ...
        ####################
        plt.figure(figsize=(25, 10))
        sns.displot(10 * torch.log10(MSE_KN_linear_arr).cpu().numpy(), hist=False, kde=True, color='g', label = self.modelName)
        ...
        sns.displot(10 * torch.log10(MSE_KF_data_linear_arr).cpu().numpy(), hist=False, kde=True, color= 'r', label = 'Kalman Filter')
