...
# r2searchdB = torch.tensor([-5,0,5], dtype=torch.float32)
# rsearch = torch.sqrt(10**(-r2searchdB/10))
# q2searchdB = torch.tensor([20,15,10], dtype=torch.float32)
# qsearch = torch.sqrt(10**(-q2searchdB/10))

### q and r optimized for filters
# r2optdB = torch.tensor([3.0103], dtype=torch.float32)
# ropt = torch.sqrt(10**(-r2optdB/10))

# r2optdB_partial = torch.tensor([3.0103], dtype=torch.float32)
# ropt_partial = torch.sqrt(10**(-r2optdB_partial/10))

# q2optdB = torch.tensor([18.2391,28.2391,38.2391,48,55], dtype=torch.float32)
# qopt = torch.sqrt(10**(-q2optdB/10))

# q2optdB_partial = torch.tensor([18.2391,28.2391,38.2391,48,55], dtype=torch.float32)
# qopt_partial = torch.sqrt(10**(-q2optdB_partial/10))

traj_resultName = ['traj_lor_KNetFull_rq020_T100_r2_100.pt']#,'partial_lor_r4.pt','partial_lor_r5.pt','partial_lor_r6.pt']
...
modelFolder = 'KNet' + '/'
KNet_Pipeline = Pipeline_EKF(strTime, "KNet", "KNet_"+ dataFileName[0])
