# -*- coding: utf-8 -*-
"""
Created on Thu Sep  8 23:43:06 2022

@author: Natalia.DaSilvaDeSa
"""
...
class KalmanBoxTracker(object):
  ...
  r2 = torch.tensor([1.], dtype=torch.float32)
  vdB = 0 # ratio v=q2/r2
  v = 10**(vdB/10)
  q2 = torch.mul(v,r2)
...
#for index in range(0,len(r2)):
  ...
  r = torch.sqrt(r2[0])
  q = torch.sqrt(q2[0])
  print ('f ', f, 'q ', q, 'h ', h, 'r ', r, 't ', T, 't test ', T_test, 'm ', m, 'n ', n )
  sys_model = SystemModel(f, q, h, r, 1, 1, m, n,"Lor")# arbitary q and r
  ...
  def __init__(self,bbox):
    ...
    self.KNet_Pipeline = Pipeline_EKF(KalmanBoxTracker.strTime, "KNet", "KNet_"+ KalmanBoxTracker.dataFileName[0])
    ...
    self.KNet_Pipeline.model = torch.load(KalmanBoxTracker.modelFolder+"model_KNet_data_lor_v20_rq020_T100.pt", map_location=torch.device('cpu'))
  ...
  def predict(self):
      ...
      if((self.x[6]+self.x[2])<=0):
        self.x[6] *= 0.0
     # self.kf.predict()
      self.x = self.KNet_Pipeline.NNTest(1, torch.from_numpy(np.reshape(self.x, (1,) + self.x.shape)).float()).detach().numpy()
      print ('x ', self.x)
      self.age += 1
