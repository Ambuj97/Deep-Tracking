...
class KalmanNetNN(torch.nn.Module):
    ...
    def InitKGainNet(self, prior_Q, prior_Sigma, prior_S):
        ...
        self.batch_size = self.n_batch # Batch size
        ...
        # GRU to track Q
        self.d_input_Q = self.m * self.in_mult_KNet
        self.d_hidden_Q = self.m ** 2
        self.GRU_Q = nn.GRU(self.d_input_Q, self.d_hidden_Q).to(self.device)

        # GRU to track Sigma
        self.d_input_Sigma = self.d_hidden_Q + self.m * self.in_mult_KNet
        self.d_hidden_Sigma = self.m ** 2
        self.GRU_Sigma = nn.GRU(self.d_input_Sigma, self.d_hidden_Sigma).to(self.device)
       
        # GRU to track S
        self.d_input_S = self.n ** 2 + 2 * self.n * self.in_mult_KNet
        ...
        self.d_output_FC5 = self.m * self.in_mult_KNet
        ...
        self.d_output_FC6 = self.m * self.in_mult_KNet
        ...
        self.d_output_FC7 = 2 * self.n * self.in_mult_KNet
