...
class SystemModel:
    ...
    def __init__(self, F, q, H, r, T, T_test, prior_Q=None, prior_Sigma=None, prior_S=None):
        ...
        if prior_Q is None:
            ...
            self.prior_Q[-1,-1] *= 0.01
            self.prior_Q[4:,4:] *= 0.01
        else:
            self.prior_Q = prior_Q
            

        if prior_Sigma is None:
            self.prior_Sigma = torch.eye(self.m, self.m)
            self.prior_Sigma[4:,4:] *= 1000. #give high uncertainty to the unobservable initial velocities
    ...
    def InitSequence(self, m1x_0, m2x_0):
        ...
        self.m1x_0 = m1x_0
        self.m1x_0[0] = random.randint(10, 100)
        self.m1x_0[1] = random.randint(10, 50)
        self.x_prev = m1x_0
        self.m2x_0 = m2x_0
    ...
    def GenerateSequence(self, Q_gen, R_gen, T):
        ...
        for t in range(0, T):
            ...
            ########################
            if self.q==0:
                xt = self.F.matmul(self.x_prev.type(torch.LongTensor)) 
               
            else:
                xt = self.F.matmul(self.x_prev.type(torch.LongTensor))
                #print ('m ', self.m)
                mean = torch.zeros([self.m]) 
                # mean = torch.ones([self.m]) 
                #print ('mean ', mean)
                ...
                # Change values for x and y
                eq[0] = torch.normal(3, 2, size=(1, 1))
                eq[1] = torch.normal(1, 2, size=(1, 1))
            ...
            # Observation Noise
            if self.r==0:
                yt = self.H.matmul(xt.type(torch.LongTensor))           
            else:
                yt = self.H.matmul(xt.type(torch.LongTensor))
                mean = torch.zeros([self.n])  
                # mean = torch.ones([self.n])            
